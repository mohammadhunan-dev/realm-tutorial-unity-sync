...
public class LeaderboardManager : MonoBehaviour
{
    ...
    // sync method:
    public static async Task<Realm> GetRealm() // Realms.Sync.User
    {
        var syncConfiguration = new SyncConfiguration("UnityTutorialPartition", RealmController.syncUser);
        return await Realm.GetInstanceAsync(syncConfiguration);
    }

    public async void setLoggedInUser(string loggedInUser) // sync makes this await
    {
        username = loggedInUser;

        //realm = Realm.GetInstance(); // LOCAL

        realm = await GetRealm(); // Sync example
